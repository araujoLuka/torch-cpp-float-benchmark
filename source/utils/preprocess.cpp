...
torch::Tensor preprocess_batch(
    ...
    bool needs_normalize = (meta.norm_mean.size() == 3 &&
                            meta.norm_std.size() == 3);
    ...
    if (needs_normalize) {
        // Mean/std na CPU
        static torch::Tensor mean_cpu = torch::tensor({
            0.0f, 0.0f, 0.0f
        }).view({1,3,1,1});

        static torch::Tensor std_cpu = torch::tensor({
            1.0f, 1.0f, 1.0f
        }).view({1,3,1,1});
        ...
        if (!initialized) {
            ...
            mean_cpu = torch::tensor({
                (float)meta.norm_mean[0],
                (float)meta.norm_mean[1],
                (float)meta.norm_mean[2],
            }).view({1,3,1,1});

            std_cpu = torch::tensor({
                (float)meta.norm_std[0],
                (float)meta.norm_std[1],
                (float)meta.norm_std[2],
            }).view({1,3,1,1});
        }
        ...
        void* dev_key = device.is_cpu() ? nullptr : device.index() >= 0 ? reinterpret_cast<void*>(static_cast<intptr_t>(device.index())) : reinterpret_cast<void*>(-1);
